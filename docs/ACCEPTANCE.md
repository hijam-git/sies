...
- [ ] **Fee idempotency** — `generate_monthly_fees` twice ⇒ ONE invoice per
      (student, category, period). The constraint that makes a retried beat job
      safe.
- [ ] **Receipt-number concurrency** — several threads collecting at once get
      distinct consecutive numbers. No gaps, no duplicates.
- [ ] **Income auto-posting is atomic** — a collection writes exactly one Income
      row; a rolled-back collection writes **neither**.
- [ ] **Money arithmetic** — part payments, `payable = amount − discount + fine`,
      over-collection refused, every status transition in `docs/06` #8.
- [ ] **Fines** — accrue while overdue, respect grace, respect the cap, stop on
      payment.
- [ ] **Attendance is idempotent and server-authoritative** — same batch twice ⇒
      same rows; a future date or off day is rejected into `skipped`, never
      written, however the client was persuaded to send it.
- [ ] **Attendance window** — inside it `attendance.take` suffices; outside it
      needs `attendance.update`.
- [ ] **Teacher scoping (D6)** — a teacher cannot mark attendance for, or enter
      marks in, a class or subject they are not assigned.
- [ ] **Timetable double-booking** — the database refuses a teacher in two rooms
      at once.
- [ ] **Marks** — one mark per (exam, student, subject); invisible before
      publish; publish is principal-only.
- [ ] **Branch isolation, every module** — a branch-A user gets **404, not 403**,
...
- [ ] **Permission fallback** — empty list ⇒ preset; explicit list wins and is
      never merged; inactive user ⇒ no permissions; a stale permission string is
      dropped.
- [ ] **`/api/me/` isolation** — a student cannot reach another student's record
      by changing an id.
- [ ] **Printed form** — `blank` renders rules where `filled` renders values;
...
- [ ] **`admit_student` atomicity** — forced failure partway writes nothing.
...
- [ ] Migrations apply to an **empty** database AND to the existing seeded one
- [ ] **Full backend suite green, in Docker, on Postgres**
- [ ] `tsc -b` · `eslint .` · `vite build` · `vitest` all clean
- [ ] No `FloatField` anywhere; money is `Decimal` end to end
- [ ] No signals (`CLAUDE.md` §4.3)
- [ ] Every V1 nav entry reaches a real screen — no `PhasePlaceholder` left on a
      feature whose backend is live
- [ ] Celery beat schedule populated and the worker consumes both queues
...
- [ ] A madrasah with its three streams and a current session
- [ ] Classes across all three streams, with sections and subjects
- [ ] A bell schedule and a weekly routine with no clashes
- [ ] Teachers **assigned** to classes and subjects — so scoping is visible
- [ ] Employees
- [ ] Students enrolled with rolls, guardians, and structured addresses
- [ ] A month of attendance already marked
- [ ] Fees generated, some collected, some overdue with fines — and the income
      ledger agreeing with the receipts **without anyone having typed it twice**
- [ ] An exam with marks entered
- [ ] **Logins handed over: a platform admin, a principal, a teacher, a student**
