...
`core/settings.py` still has `exams` commented out of `_SIES_APPS` — the phase
...
Until then the tests run as:
    ...
    scripts/dev.sh test exams --settings=exams.tests.settings
...
# Apps that exist on disk but are still commented out of `_SIES_APPS`. `exams`
...
# with exams. Discovered rather than hard-listed, so it needs no edit as the
